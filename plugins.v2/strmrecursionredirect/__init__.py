...
class StrmRecursionRedirect(_PluginBase):
    # 插件名称
    plugin_name = "Strm重定向(递归)"
    # 插件描述
    plugin_desc = "重写所有递归文件夹及子文件夹下的Strm文件内容"
    # 插件图标
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "strmrecursionredirect_"
    # 加载顺序
    plugin_order = 27
    # 可使用的用户级别
    auth_level = 1
    ...
    def update_strm(self, target_from, target_to, directory):
      ...
      for root, dirs, files in os.walk(directory):
          for name in files:  # 只遍历文件
              file_path = os.path.join(root, name)
              # 过滤出 .strm 文件
              if Path(file_path).suffix.lower() == ".strm":
                  try:
                      ...
                      # 重定向逻辑
                      if target_from and target_to and unercoded_strm_content.startswith(target_from):
                          strm_content = unercoded_strm_content.replace(target_from, target_to)
                          
                          # 可能需要调整编码逻辑
                          no_encoded = self.find_unencoded_parts(unercoded_strm_content)[0]  # 获取未编码的部分
                          ...
                          with open(file_path, 'w', encoding='utf-8') as file:
                              ...
                              logger.info(f"Updated Strm: {unercoded_strm_content} -> {strm_content} success")
